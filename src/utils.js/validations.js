...
export const signUpValidation = (input) => {
  ...
  const passwordResult = Joi.validate(
  ...
  const secQuestionResult = Joi.validate(
    { securityQuestion: input.securityQuestion },
    securityQuestionSchema
  );
  const secAnswerResult = Joi.validate(
  ...
  if (passwordResult.error !== null) {
    errors.password =
      "Password should not be empty and should have minimum of 6 characters";
  }
